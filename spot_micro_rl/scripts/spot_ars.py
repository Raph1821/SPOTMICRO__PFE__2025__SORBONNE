...
import time
from src.ars_lib.ars import ARSAgent, Normalizer, Policy, ParallelWorker
...
def main():
    ...
    # Start multiprocessing (on Windows)
    # Each worker creates its own env (Windows spawn requirement)
    env_kwargs = dict(render=False, on_rack=False, height_field=True,
                      draw_foot_path=False)
    for proc_num in range(num_processes):
        p = mp.Process(target=ParallelWorker,
                       args=(childPipes[proc_num], spotBezierEnv, env_kwargs,
                             state_dim))
        p.start()
        processes.append(p)
