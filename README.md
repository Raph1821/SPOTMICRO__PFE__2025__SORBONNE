...
#### Software:
This repo is structured as a catkin workspace in a ROS Noetic environment on Ubuntu 20.04. The software is designed to work with this modern environment. Raspberry Pi images with Ubuntu 20.04 can be found at the [official Ubuntu website](https://ubuntu.com/download/raspberry-pi). It is suggested to also install ROS Noetic on a Ubuntu 20.04 linux installation/dual boot/virtual machine on a PC for development and for running control nodes. Instructions to install ROS Noetic can be found [here](http://wiki.ros.org/noetic/Installation/Ubuntu).

**NOTE**  With the Raspberry Pi 4B 8GB, a SWAP partition is **NOT necessary** due to the abundant 8GB RAM. The catkin compilation process will run smoothly without any additional virtual memory configuration.
...
Three additional ROS packages may need to be installed for this project to build successfully. They can be installed via:
```
...
This section attempts to be a full set of instructions to get a spot micro robot calibrated and running with this code.
...
#### Running:
Open at least two terminal windows, with at least one ssh'ed to the raspberry pi. I recommend using a terminal multiplexer such as `tmux` for convenience. Start the following launch files in the respective terminals:
* `roslaunch spot_micro_motion_cmd motion_cmd.launch`: Run on the Raspberry Pi. Launches the i2c_pwmboard node as well as the robot's motion control node. On startup, the motion command node sends a servo configuration message to the i2c_pwmboard node, then starts a state machines and enters an idle state.
...
* **OPTIONAL**: The above two launch files can take optional command line arguments to start additional nodes. Some of the command line arguments are listed below.
    ...
    * Command line arguments for `keyboard_command.launch`:
        ...
        * `run_plot:=true`: Runs python plotting node to display a stick figure wireframe model of the spot micro robot state in real time. Must be run on a PC. Requires matplotlib (version 3.3.0+) and numpy (version 1.19.0+) for Python 3.
...
The launch files are an included convenience, but if needed, nodes can be run individually via `rosrun` (except for spot_micro_motion command which must be launched by launch file to read in parameters). For example, the minimum required nodes could be run with the following three commands in three separate terminals:
...
The robot software is driven by a finite state machine of discrete control modes. On the terminal running the spot_micro_keyboard_command node, keyboard commands are issued to move through the state machine and also to command body rates and angle commands.

After all software is started, type in and issue `stand` command in the keyboard control node terminal to command the robot to stand up. From here, either an `idle` command can be issued to sit back down and set the servos to idle, or `angle_cmd` can be issued to command body orientation angles, or `walk` can be issued to enter walk mode.
...
Note that the software does not currently support any command limits so deleterious or even hardware damaging behavior may be experienced if commanding orientations or body rates beyond the capability of the robot.

Typing `quit` at the command input will quit the program with the servos fixed at their previous commanded value. Thus it is recommended to put the robot in idle before quitting. In idle mode servos are commanded such that they hold no fixed position and can "free wheel".
...
The default gait is a walk style gait that consists of 8 phases, swings only one leg at a time, and shifts the body in between leg swings to balance the body over the 3 legs that remain on the ground. No speed or angle command limits are implemented in the software, but the inverse kinematics model does crude trigonometric domain function limiting to avoid math errors.

A yaml configuration file is used for holding various software configuration settings, including servo configuration dictionaries. Servo's can be calibrated using the servo_move_keyboard node, a calibration of angles via eye is sufficient for the reasonable performance.
...
* **spot_micro_joy**: Sends the same commands like the keyboard_command_node but is controlled by sensor_msgs/Joy, which are emitted by joy_node. By default it is configured for PS4 button-layout. Make sure to take a look into the [joystick control](docs/joystick_control.md) documentation before trying.
...
* **servo_move_keyboard**: A python node that can be used in conjunction with the i2cpwm_board node to manually command an individual servo via keyboard controls. Can be used for servo calibration to build the servo configuration dictionary.
